...
public class User implements UserDetails {    // Classe  do banco -> User | Representa os usuários do sistema.
    ...
    @CreationTimestamp // O momento de registro será dado assim que o objeto for criado.
    ...
    @JoinTable(name = "tb_users_roles", // Nome da tabela que será criada
            joinColumns = @JoinColumn(name = "user_id"), // nome da coluna do registro do id do user (PK)
            inverseJoinColumns = @JoinColumn(name = "role_id")) // nome da coluna do registro do id da role (PK)
